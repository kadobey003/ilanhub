...
export async function fetchJobsCategory(projectId: string) {
  const json = await authRequest<{ data: ApiCategory }>(
    `/api/account/projects/${projectId}/jobs/category`,
  );
  return json.data;
}
...
export interface JobsListingPayload {
  projectId: string;
  categoryId: string;
  cityId: string;
  businessType?: string;
...
export async function createJobsListing(payload: JobsListingPayload) {
  return authRequest<{ data: { id: string; status: string; price: number | null } }>(
    "/api/account/listings/jobs",
