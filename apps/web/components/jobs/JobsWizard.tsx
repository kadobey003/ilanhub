...
export function JobsWizard() {
  ...
  useEffect(() => {
    if (!isLoggedIn()) {
      router.replace("/login?from=/create?project=jobs");
      return;
    }
  ...
  const previewText = useMemo(() => {
    const adminFooter: string[] = [
      t("bot.jobs.basePrice", { price: formatAmountUah(bundlePrice) }),
    ];
    if (pinPost) {
      adminFooter.push(
        t("bot.jobs.addonPin", { price: formatAddonUah(pinPrice) }),
      );
    }
    if (dailyDuplicate) {
      adminFooter.push(
        t("bot.jobs.addonDaily", { price: formatAddonUah(dailyPrice) }),
      );
    }
    adminFooter.push(
      t("bot.jobs.totalPrice", { price: formatAmountUah(totalPrice) }),
    );
    if (scheduledPostAt) {
      adminFooter.push(`📅 Публікація: ${scheduledPostAt}`);
    }

    return formatJobsPreview(
      {
        businessType,
        title,
        address,
  ...
  const stepPrompt = useMemo(() => {
    switch (step) {
      case HorecaStep.SELECT_CITY:
        return `${i18n.bot.jobs.intro}\n\n${i18n.bot.jobs.selectCity}`;
      case HorecaStep.BUSINESS_TYPE:
        return i18n.bot.jobs.businessType;
      case HorecaStep.BUSINESS_NAME:
        return i18n.bot.jobs.businessName;
      case HorecaStep.ADDRESS:
        return i18n.bot.jobs.address;
      case HorecaStep.VACANCY_COUNT:
        return i18n.bot.jobs.vacancyCount;
      case HorecaStep.VACANCY_TITLE:
        return t("bot.jobs.vacancyTitle", {
          n: vacancyIndex + 1,
          total: vacancyCount,
        });
      case HorecaStep.VACANCY_EXPERIENCE:
        return t("bot.jobs.vacancyExperience", { title: vacancyLabel });
      case HorecaStep.VACANCY_SALARY:
        return t("bot.jobs.vacancySalary", { title: vacancyLabel });
      case HorecaStep.VACANCY_SCHEDULE:
        return t("bot.jobs.vacancySchedule", { title: vacancyLabel });
      case HorecaStep.VACANCY_TIME:
        return t("bot.jobs.vacancyTime", { title: vacancyLabel });
      case HorecaStep.VACANCY_DESCRIPTION:
        return t("bot.jobs.vacancyDescription", { title: vacancyLabel });
      case HorecaStep.BENEFITS:
        return i18n.bot.jobs.benefits;
      case HorecaStep.CONTACT:
        return i18n.bot.jobs.contact;
      case HorecaStep.UPLOAD_PHOTOS:
        return i18n.bot.jobs.uploadPhotos;
      case HorecaStep.PIN_POST:
        return t("bot.jobs.pinPost", { price: formatAddonUah(pinPrice) });
      case HorecaStep.SCHEDULE_POST:
        return i18n.bot.jobs.schedulePost;
      case HorecaStep.DAILY_DUPLICATE:
        return t("bot.jobs.dailyDuplicate", { price: formatAddonUah(dailyPrice) });
      case HorecaStep.PREVIEW:
        return i18n.bot.jobs.preview;
      default:
        return "";
    }
  ...
  const submit = async () => {
    ...
    setError("");
    try {
      const res = await createJobsListing({
        projectId,
        categoryId,
        cityId,
        businessType,
    ...
    <div className="space-y-5">
      ...
      {step === HorecaStep.BUSINESS_TYPE && (
        <input
          ...
          placeholder="Офіс, IT, виробництво…"
        />
      )}
      ...
      {step === HorecaStep.VACANCY_TITLE && (
        <input
          ...
          placeholder="Менеджер, водій, програміст…"
        />
      )}
      ...
      {step === HorecaStep.UPLOAD_PHOTOS && (
        <div className="space-y-3">
          ...
          {mediaUrl && !photoUploading && (
            <p className="text-sm text-green-600">{i18n.bot.jobs.photoAdded}</p>
          )}
        </div>
      )}
          ...
          <button
            ...
            onClick={() => {
              if (scheduledPostAt && !parseScheduleDate(scheduledPostAt)) {
                setError(i18n.bot.jobs.invalidDate);
                return;
              }
              setError("");
              setStep(HorecaStep.DAILY_DUPLICATE);
